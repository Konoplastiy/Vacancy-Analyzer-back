...
public class NewsletterSubscriberService {
    ...
    @Transactional
    public NewsletterSubscriber saveNewsletterSubscriber(@NonNull final NewsletterSubscriber subscriber) {
        final String email = subscriber.getEmail();
        newsletterSubscriberRepository.findByEmail(email).ifPresent(s -> {
            throw new SubscriberAlreadyExistsException(String.format(SUBSCRIBER_ALREADY_EXITS_MESSAGE, email));
        });
